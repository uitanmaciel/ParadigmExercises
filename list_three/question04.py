def __enunciate():
    print('''# Questão 04 - Uma loja utiliza o código “V” para transação à vista e “P” para transação à prazo. 
# Faça um algoritmo que receba o código e o valor de 15 transações. Calcule e mostre:
#
...
def entry():
    transactions = []
    while True:
        code = input("Informe o código da transação (V para à vista, P para à prazo ou S para SAIR): \n")
        if code == "S" or code == "s":
            break
...
def total_value_cash(transactions):
    total = 0
    for transaction in transactions:
        if transaction[0] == "V" or transaction[0] == "v":
            total += transaction[1]
    return total
...
def total_value_installments(transactions):
    total = 0
    for transaction in transactions:
        if transaction[0] == "P" or transaction[0] == "p":
            total += transaction[1]
    return total
